...
- Track free-tier usage with /usage/check and /usage/increment
"""
...
class UserPublic(BaseModel):
    ...
    total_guesses: int = 0
    total_correct: int = 0
    acc_guessing_ai: int = 0
    acc_guessing_real: int = 0
...
async def lifespan(app: FastAPI):
    ...
    mongo_client = AsyncIOMotorClient(_require_setting("MONGO_URI", MONGO_URI))

    db_name = MONGO_DB 
    app_db = mongo_client[db_name]

    users_coll = app_db["users"]
    api_keys_coll = app_db["api_keys"]
...
def build_user_public(doc: dict) -> UserPublic:
    return UserPublic(
        ...
        age=doc.get("age"),
        total_guesses=int(doc.get("total_guesses", 0)),
        total_correct=int(doc.get("total_correct", 0)),
        acc_guessing_ai=int(doc.get("acc_guessing_ai", 0)),
        acc_guessing_real=int(doc.get("acc_guessing_real", 0)),

        monthly_usage_count=int(doc.get("monthly_usage_count", 0)),
...
async def delete_me(user: TokenUser = Depends(get_current_user)):
    ...
    return {"deleted": True, "user_id": result["user_id"], "message": "Your account has been permanently deleted"}


# -------- Usage tracking (free tier) --------

@app.post("/usage/check")
async def check_usage(user: TokenUser = Depends(get_current_user)):
    assert users_coll is not None
    ...
    if plan != 0:
        return {"allowed": True, "plan": plan, "unlimited": True}
    ...
    # ---- FIX: normalize usage_reset_date to timezone-aware UTC ----
    if isinstance(usage_reset_date, datetime):
        if usage_reset_date.tzinfo is None:
            # Mongo returned naive datetime -> assume it's UTC
            usage_reset_date = usage_reset_date.replace(tzinfo=timezone.utc)
        else:
            # ensure it's in UTC for consistent comparisons
            usage_reset_date = usage_reset_date.astimezone(timezone.utc)
    # --------------------------------------------------------------

    if usage_reset_date is None or now >= usage_reset_date:
        next_reset = now.replace(day=1, hour=0, minute=0, second=0, microsecond=0) + timedelta(days=32)
...
async def increment_usage(user: TokenUser = Depends(get_current_user)):
    assert users_coll is not None
    ...
    plan = int(doc.get("plan", 0))
    if plan != 0:
        return {"incremented": False, "message": "Unlimited plan - no tracking needed"}

    await users_coll.update_one({"user_id": user.user_id}, {"$inc": {"monthly_usage_count": 1}})
    updated = await users_coll.find_one({"user_id": user.user_id})
    return {"incremented": True, "monthly_usage": int(updated.get("monthly_usage_count", 0))}
