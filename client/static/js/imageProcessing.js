    ...
    btnCheck.addEventListener("click", async () => {
      ...
      try {
        ...
        if (res.ok) {
          ...
          // Usage limit exceeded (merged behavior from v2)
          lastDetectionToken = null;
          if (btnSave) btnSave.disabled = true;
          if (saveState) saveState.textContent = "Run detection first to enable saving.";

          const errorDetail = data.detail || {};
          const message = errorDetail.message || "You've reached your free tier limit. Upgrade to premium for unlimited scans.";

          if (detectResult) {
            detectResult.innerHTML = `
              <div style="color: #ff6b6b; padding: 15px; background: #ffe0e0; border-radius: 5px; margin-top: 10px;">
                <strong>⚠️ Usage Limit Reached</strong>
                <p>${message}</p>
                <p>Used: ${errorDetail.monthly_usage || 0}/${errorDetail.limit || 10} scans this month</p>
                <a href="/plan" style="display: inline-block; margin-top: 10px; padding: 10px 20px; background: #4CAF50; color: white; text-decoration: none; border-radius: 5px;">
                  Upgrade to Premium
                </a>
              </div>
            `;
          }
          setStatus(detectStatus, "error", "Free tier limit reached. Please upgrade.");
        } else {
          lastDetectionToken = null;
          if (btnSave) btnSave.disabled = true;
          if (saveState) saveState.textContent = "Run detection first to enable saving.";
