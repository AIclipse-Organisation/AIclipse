...
function makeNotificationCard(item) {
  ...
  if (item?.type === "vote") {
    ...
    if (vote === "real") {
      icon.src = assetUrl("images/upvote.png");
    } else {
      icon.src = assetUrl("images/downvote.png");
    }
  } else if (item?.type === "moderation") {
    icon.src = assetUrl("images/lock.png");
    icon.style.opacity = "0.8";
  } else {
    icon.src = assetUrl("images/comment.png");
  }
...
function renderList(items) {
  ...
  if (!Array.isArray(items) || items.length === 0) {
    ...
    emptyIcon.src = assetUrl("images/no_notifications.png");
