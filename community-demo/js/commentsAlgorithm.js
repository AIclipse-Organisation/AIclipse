function calculateCommentScores(posts, comments) {
  const commentsMap = Object.fromEntries(comments.map(c => [c.comment_id, c]));
  const repliesCountMap = {};

  comments.forEach(c => {
    if (c.parent_comment_id) {
      repliesCountMap[c.parent_comment_id] = (repliesCountMap[c.parent_comment_id] || 0) + 1;
    }
  });
  ...
  posts.forEach(post => {
    const scoredComments = (post.comments_id || []).map(cid => {
      ...
      return { comment_id: comment.comment_id, score };
  ...
  return postCommentScores; // { postId: totalCommentScore }
}
