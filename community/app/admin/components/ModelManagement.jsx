        ...
        <div className="flex items-center gap-3">
          <div className="flex bg-black/40 p-2 rounded-3xl border border-white/5 shadow-xl">
            <Button 
              ...
              className={`font-black px-8 rounded-2xl transition-all uppercase text-[10px] tracking-widest h-12 ${
                activeView === "models" ? "bg-[#CFB87C] text-[#222222] shadow-lg shadow-[#CFB87C]/10" : "bg-transparent text-gray-500 hover:text-white"
              }`}
            >
              Models
            </Button>
            <Button 
              ...
              className={`font-black px-8 rounded-2xl transition-all uppercase text-[10px] tracking-widest h-12 ${
                activeView === "data" ? "bg-[#CFB87C] text-[#222222] shadow-lg shadow-[#CFB87C]/10" : "bg-transparent text-gray-500 hover:text-white"
              }`}
            >
              Data
            </Button>
                ...
                <CardBody className="p-8 text-white">
                  <form onSubmit={handleUploadSubmit} className="flex flex-col md:flex-row gap-8 items-center justify-between">
                    <div className="flex flex-col gap-2 w-full md:w-1/3">
                      <label className="text-[10px] font-black text-gray-400 uppercase tracking-widest px-2">Version</label>
                      <Input 
                    ...
                    <div className="flex flex-col gap-2 w-full md:w-1/3 items-center">
                       <label className="text-[10px] font-black text-gray-400 uppercase tracking-widest">Model File</label>
                       <input 
                         type="file" 
                         ref={fileInputRef} 
                         className="block text-[11px] font-black uppercase text-gray-400 file:mr-4 file:py-2 file:px-4 file:rounded-xl file:border-0 file:bg-white/10 file:text-[#CFB87C] cursor-pointer" 
                       />
                    </div>
                    ...
                    <Button 
                      ...
                      className={`font-black uppercase text-[11px] tracking-widest h-12 px-10 rounded-2xl transition-all ${
                        uploadVersion ? "bg-[#CFB87C] text-[#222222] shadow-lg shadow-[#CFB87C]/10" : "bg-white/5 text-gray-400"
                      }`}
                    >
                      Upload Model
                    </Button>
                  ...
                  <Card key={model.id} className="border border-white/5 bg-[#1a1a1a] shadow-xl rounded-[2.5rem] p-4 text-white">
                    <CardHeader className="p-4 pb-0 flex justify-between items-start">
                      <div>
                         ...
                         <p className="text-[11px] font-bold text-gray-400 uppercase tracking-widest mt-1">{new Date(model.createdAt).toLocaleString()}</p>
                      </div>
                      <div className="flex items-center gap-2">
                        {model.isDeployed ? (
                          <Chip className="bg-[#CFB87C] text-[#222222] font-black uppercase text-[10px] tracking-widest px-3 border-none shadow-lg shadow-[#CFB87C]/10">ACTIVE</Chip>
                        ) : model.rejectionReason ? (
                          <Chip className="bg-danger/20 text-danger font-black uppercase text-[10px] tracking-widest px-3 border border-danger/20">REJECTED</Chip>
                        ) : (
                          <Chip className="bg-white/5 text-gray-400 font-black uppercase text-[10px] tracking-widest px-3 border border-white/5">ARCHIVED</Chip>
                        )}
                      </div>
                    </CardHeader>
                    ...
                    <CardBody className="gap-6 p-4 py-6 flex flex-col">
                      ...
                      <div className="p-4 bg-blue-500/5 rounded-2xl border border-blue-500/10">
                        <div className="flex justify-between items-center mb-3">
                          <span className="text-[11px] font-black text-blue-400 uppercase tracking-widest">Golden Test Set</span>
                        ...
                        <div className="grid grid-cols-2 gap-4 mb-3">
                          <div className="flex flex-col">
                            <span className="text-[10px] text-gray-400 font-black uppercase">F1 Score</span>
                            <span className="text-base font-black text-white">{(model.goldenTestF1Score).toFixed(3)}</span>
                          </div>
                          <div className="flex flex-col text-right">
                            <span className="text-[10px] text-gray-400 font-black uppercase">Recall</span>
                      ...
                      <div>
                        <div className="flex justify-between items-end mb-2 px-1 text-[11px] font-black uppercase tracking-widest">
                      ...
                      <div className="flex justify-between items-center pt-3 border-t border-white/10">
                        <div className="flex gap-4">
                          <div className="flex flex-col">
                              <span className="text-[10px] text-gray-400 font-black uppercase">New Data</span>
                              <span className="text-sm font-black text-white">{model.newImagesCount} <span className="text-[10px] text-gray-400">IMG</span></span>
                          </div>
                          <div className="flex flex-col border-l border-white/10 pl-4">
                              <span className="text-[10px] text-gray-400 font-black uppercase">Replay</span>
                              <span className="text-sm font-black text-white">{model.replayBufferCount} <span className="text-[10px] text-gray-400">IMG</span></span>
                          </div>
                        </div>
                      </div>
                    ...
                    <CardFooter className="justify-end p-4 pt-0">
                      <Button 
                        size="sm" 
                        variant="light" 
                        className="font-black uppercase text-[10px] tracking-widest text-gray-400 hover:text-danger hover:bg-danger/10 transition-colors"
