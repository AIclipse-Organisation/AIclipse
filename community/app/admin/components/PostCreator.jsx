...
export default function PostCreator() {
  ...
  return (
    <Card className="w-full h-full border border-white/5 shadow-2xl rounded-[3rem] bg-[#1a1a1a] overflow-hidden text-white">
      <div className="px-10 py-4 text-[9px] tracking-[0.4em] uppercase font-black border-b border-white/5 flex justify-between items-center bg-black/20 text-[#CFB87C]">
        ...
        {isSuccess ? (
          ...
          <form onSubmit={handleAdminPublish} className="grid grid-cols-12 gap-12 flex-1">
            <div className="col-span-12 lg:col-span-5 flex flex-col gap-4">
              <label className="text-[10px] font-black text-gray-500 uppercase tracking-widest px-2">Evidence Media</label>
              <div className="relative aspect-square rounded-[2.5rem] bg-black/40 border-2 border-dashed border-white/10 overflow-hidden flex items-center justify-center group">
                {!selectedFile ? (
                  <div className="flex flex-col items-center gap-4 cursor-pointer" onClick={() => fileInputRef.current?.click()}>
                    ...
                    <Button className="bg-[#CFB87C] text-[#222222] font-black px-10 h-12 rounded-2xl uppercase text-[10px] tracking-widest pointer-events-none">Browse</Button>
                  </div>
                ) : (
            ...
            <div className="col-span-12 lg:col-span-7 flex flex-col gap-8 h-full">
              <div className="flex flex-col gap-4">
                <label className="text-[10px] font-black text-gray-500 uppercase tracking-widest px-2">Truth Verdict</label>
                ...
                <div className="flex bg-black/40 p-1.5 rounded-[2rem] border border-white/5">
                  <button type="button" onClick={() => setGroundTruth("Real")} 
                    className={`flex-1 h-14 rounded-[1.5rem] font-black uppercase text-[10px] tracking-widest transition-all ${groundTruth === "Real" ? "bg-[#CFB87C] text-[#222222]" : "text-gray-500 hover:text-white"}`}>
                    REAL
                  </button>
                  <button type="button" onClick={() => setGroundTruth("AI")} 
                    className={`flex-1 h-14 rounded-[1.5rem] font-black uppercase text-[10px] tracking-widest transition-all ${groundTruth === "AI" ? "bg-red-500 text-white" : "text-gray-500 hover:text-white"}`}>
                    FAKE
                  </button>
                </div>
              </div>
              ...
              <div className="flex flex-col gap-4 flex-grow">
                <label className="text-[10px] font-black text-gray-500 uppercase tracking-widest px-2 text-center">Post Description</label>
                <Textarea 
              ...
              <div className="mt-auto pb-4">
                <Button type="submit" isDisabled={!isFormComplete || isPublishing} 
                  className={`w-full font-black text-xl h-24 rounded-[2.5rem] transition-all duration-500 ${
                    isFormComplete ? "bg-[#CFB87C] text-[#222222] shadow-xl shadow-[#CFB87C]/10 hover:-translate-y-1" : "bg-white/5 text-gray-600"
                  }`}>
